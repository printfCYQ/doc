...
1. 建库建表：用[第 2 章](#_2-建库建表-先有容器-再放东西)的三张表，顺便加上外键约束（2.5）。
...
| 步骤 | 建议章节 | 内容 / 时长 |
| --- | --- | --- |
| 第 1 步 | [第 0~1 章](#_0-数据库到底是什么) | 搞懂概念 + 跑起来（半天） |
| 第 2 步 | [第 2~3 章](#_2-建库建表-先有容器-再放东西) | 建表 + CRUD + 约束/权限（1~2 天），重点，反复练 |
| 第 3 步 | [第 4 章](#_4-查询进阶-从会查到查得对) | 查询进阶 JOIN/分组/视图/函数（2~3 天），难点，多写 |
| 第 4 步 | [第 5 章](#_5-事务-为什么钱不能凭空消失) | 事务（1 天） |
| 第 5 步 | [第 6~7 章](#_6-索引-让查询快起来的目录) | 索引 + 优化（2~3 天），面试常考 |
| — | — | 以上能应付日常开发 |
| 第 6 步 | [第 8 章](#_8-原理进阶-可选-面试或深入用) | 原理进阶（按需） |
| 第 7 步 | [第 9~14 章](#_9-备份与恢复-生产必备) | 备份/高可用/分库分表/运维/实战（工作中遇到再深） |
