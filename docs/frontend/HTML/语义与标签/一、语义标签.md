...
| &lt;keygen> | 提供一种验证用户的可靠方法&lt;br>生成一个公钥和私钥 |
...
> 1.  与其他图像格式相比，是哟个SVG的优势在于：

> 2.  SVG图像可通过文本编译器来创建和修改

> 3.  SVG图像可被搜索、索引、脚本化或压缩

> 4.  SVG是可伸缩的

> 5.  SVG图像可在任何的分辨率下被高质量的打印
...
1.  websorage拥有5M的存储容量，而cookie却只有4K，这是完全不能比的。
2.  客户端存储数据有两个对象，其用法基本是一致。
